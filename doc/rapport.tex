...
object. Body's attributes and functions can be seen in figure XX. The sun is very similar to the planets except that it also ``emits sun light'',and is therefore a subclass that inherits from
body (see figure YY).  Sunlight is emitted by uploading the current position of the sun to the fragment shader where the light on all other objects are calculated. The sun is highlighted by a constant light, same as the SPACEbox.
